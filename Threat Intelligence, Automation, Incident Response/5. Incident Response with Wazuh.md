In the dynamic landscape of cybersecurity, having a rapid and efficient incident response (IR) plan is crucial to address security events effectively. For instance, a sales employee opening a malicious file disguised as an email attachment from a trusted partner could lead to a ransomware attack, disrupting mission-critical services. Prompt response in such scenarios minimizes network damage, reduces downtime, and lowers associated costs, enabling businesses to quickly resume normal operations.
...
Isolating a compromised endpoint is a key part of Incident Response (IR) in a SOC to prevent threats from spreading and causing further harm. Before isolating, it’s important to assess the compromise severity, asset value, and business impact, as isolation is not a one-size-fits-all solution. Ransomware attacks—where malware encrypts data and demands payment—highlight the need for isolation, as ransomware can spread rapidly across a network. This section focuses on isolating a Windows machine after a malware infection using Wazuh’s active response feature to automatically block all outbound traffic. Covered topics include:
...
**Step 1:** Configure VirusTotal API key
On Wazuh manager, edit the `/var/ossec/etc/ossec.conf` file and add the following configuration to enable VirusTotal integration:
...
**Step 2:** Create a custom rule for Windows
Edit the `/var/ossec/etc/rules/local_rules.xml` file and add the following rule to detect a VirusTotal alert for a Windows machine:
...
<group name="syscheck,pci_dss_11.5,nist_800_53_SI.7,">
  ...
  <rule id="100200" level="7">
    <if_sid>550</if_sid>
    <field name="file">C:\Users\I3Cwg\Downloads</field>
    <description>File modified in C:\Users\I3Cwg\Downloads.</description>
  </rule>
  <rule id="100201" level="7">
    <if_sid>554</if_sid>
    <field name="file">C:\Users\I3Cwg\Downloads</field>
    <description>File added to C:\Users\I3Cwg\Downloads.</description>
  </rule>
</group>
```

**Step 3:** Configure File Integrity Monitoring (FIM) on Windows

Open configuration file `C:\Program Files (x86)\ossec-agent\ossec.conf` and add the following configuration to monitor the Downloads folder:
...
After making the changes, restart the Wazuh agent to apply the new configuration.
